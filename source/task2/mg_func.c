...
void Multigrid(int gamma, int nPoints, double totalLength, \
double** grid, double** source, FILE *fLog)
{
  int i,j,k,x,y;
  ...
  if ( nPoints <= COARSESTPOINTS ) {
    ...
    fprintf(fLog,"%d\t0\n", nPoints);
    for ( k = 0 ; k < gamma ; k++) {
      Multigrid(gamma, nCoarsePoints, totalLength, v, \
      coarseResidual, fLog);
      fprintf(fLog,"%d\t0\n", nPoints);
    }
...
double GaussSeidel(int nPoints, double totalLength, double** grid, \
double** rho, FILE* fLog) {
  ...
  for ( x = 1 ; x < nPoints - 1 ; x++ ) {
    for ( y = 1 ; y < nPoints - 1 ; y++ ) {
      oldValue = grid[x][y];
      grid[x][y] = 1.0/4.0 * (grid[x+1][y] + grid[x-1][y] + grid[x][y+1] \
      + grid[x][y-1] - pow(cellLength,2)*rho[x][y]);
